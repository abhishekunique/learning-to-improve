...
class TrajectoryImproverMLP(nn.Module):
    """MLP model for context-based action prediction."""
    
    def __init__(self, context_length: int = 20, context_dim: int = 7, state_dim: int = 4, 
                 action_dim: int = 2, hidden_dim: int = 128, num_layers: int = 4):
        super().__init__()
        
        # Input: context (context_length * context_dim) + state (state_dim)
        input_dim = context_length * context_dim + state_dim
        
        # Build network layers dynamically
        layers = []
        layer_sizes = [input_dim] + [hidden_dim] * num_layers + [action_dim]
    ...
    def forward(self, context_and_state):
        """
        Predict action given context and current state.
        
        Args:
            context_and_state: Concatenated tensor of context and state
                              Shape: (batch_size, context_length * context_dim + state_dim)
        
        Returns:
            Predicted action: Shape: (batch_size, action_dim)
        """
        return self.network(context_and_state)
...
def train_trajectory_improver(bad_trajectories: List[List[Dict]], 
                             ...
                             num_samples_per_traj: int = 5) -> TrajectoryImproverMLP:
    """
    Train the trajectory improvement MLP with context-based action prediction.
    ...
    # Initialize model
    model = TrajectoryImproverMLP(context_length=context_length, context_dim=7, 
                                 state_dim=4, action_dim=2, hidden_dim=128, num_layers=4)
    ...
    print(f"Model input dim: {context_length * 7 + 4}, Output dim: 2")
    ...
    for epoch in epoch_pbar:
        ...
        for batch_idx, (batch_context, batch_states, batch_actions) in enumerate(batch_pbar):
            ...
            # Flatten context and concatenate with states
            batch_size = batch_context.shape[0]
            flattened_context = batch_context.view(batch_size, -1)  # Shape: (batch_size, context_length * 7)
            context_and_state = torch.cat([flattened_context, batch_states], dim=1)
            
            predicted_actions = model(context_and_state)
        ...
        with torch.no_grad():
            for batch_context, batch_states, batch_actions in val_dataloader:
                # Flatten context and concatenate with states
                batch_size = batch_context.shape[0]
                flattened_context = batch_context.view(batch_size, -1)
                context_and_state = torch.cat([flattened_context, batch_states], dim=1)
...
def improve_trajectory(model: TrajectoryImproverMLP, 
    ...
    """
    Improve a single trajectory using the trained model with closed-loop execution.
    ...
    with torch.no_grad():
        # Extract features for initial trajectory
        # Extract features and flatten full trajectory as context
        features_list = extract_trajectory_features(trajectory)
        context_tensor = torch.FloatTensor(features_list).unsqueeze(0)  # Shape: (1, num_steps, 7)
        flattened_context = context_tensor.view(1, -1)  # Shape: (1, num_steps * 7)
        ...
        for i in range(len(trajectory)):
            ...
            # Concatenate fixed context with current state
            context_and_state = torch.cat([flattened_context, state_tensor], dim=1)
            
            # Get model prediction for this step
            predicted_action = model(context_and_state).squeeze(0).numpy()  # Shape: (2,)
...
def main():
    ...
    if model_path is not None and os.path.exists(model_path):
        ...
        model = train_trajectory_improver(
            ...
            epochs=500, batch_size=128, learning_rate=1e-3,
    ...
    optimal_demo = generate_optimal_demonstrations(env, target=np.array([2.5, 2.5]), num_demos=1, max_episode_length=20)[0]
    ...
    print("\n=== Visualizing Results ===")
    visualize_trajectory_comparison(env, noisy_demo, improved_demo, trajectory_list, save_path='plots/trajectory_comparison_improvement_policy.png')
    ...
    visualize_trajectory_comparison(env, random_trajectory, improved_random, trajectory_list, save_path='plots/random_trajectory_improvement_policy.png')
