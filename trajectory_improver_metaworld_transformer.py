#!/usr/bin/env python3
"""
Trajectory Improvement using Transformer with Noise-Augmented Demonstrations for Meta-World
...
def extract_step_features(step: Dict[str, Any]) -> np.ndarray:
    ...
    Returns:
        Feature array: [pos_x, pos_y, pos_z, goal_x, goal_y, goal_z, action_x, action_y, action_z, l2_distance_to_goal]
    """
    pos = step['obs'][:3]
    goal = step['obs'][3:6]
...
class TrajectoryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return (
            torch.FloatTensor(self.bad_trajectories[idx]),  # Shape: (seq_len, 10)
            torch.FloatTensor(self.good_trajectories[idx])  # Shape: (seq_len, 10)
        )
...
def improve_trajectory(model: TrajectoryImproverTransformer, 
    ...
    with torch.no_grad():
        ...
        for i, step in enumerate(trajectory):
            ...
            # improved_step['obs'][3:6] = trajectory[-1]['obs'][3:6] # Replace goal with original goal
            # TODO: Maybe replace goals here
...
def main():
    ...
    from tensorboardX import SummaryWriter
    writer = SummaryWriter(f'runs/trajectory_improver_transformer_metaworld_{datetime.now().strftime("%Y%m%d")}')
    
    # Try to load existing model first
    model_path = 'models/trajectory_improver_transformer_metaworld.pt'
    load_model = True
    ...
    for i in range(num_eval_trajectories):
        ...
        visualize_trajectory_comparison(env, noisy_demo, improved_demo, trajectory_list, save_path=f'plots/mw_transformer_trajectory_comparison_improvement_{i}.png', overall_mins=overall_mins, overall_maxs=overall_maxs)
        ...
        visualize_trajectory_comparison(env, random_demo, improved_random, trajectory_list, save_path=f'plots/mw_transformer_trajectory_comparison_improvement_random_{i}.png', overall_mins=overall_mins, overall_maxs=overall_maxs)
